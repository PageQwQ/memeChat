...
/**
 * Meme picker panel rendered on top of the chat screen (26.1.2).
 * The grid sits on the left, the resource pack list on the right; clicking
 * a meme inserts ":name:" into the chat box via the callback and closes the panel.
 */
public class MemePickerPanel {
    ...
    public void render(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float deltaTicks) {
        ...
        graphics.outline(panelX, panelY, panelWidth, panelHeight, 0xff000000);
        ...
        for (String pack : packs) {
            ...
            String displayName = MemechatEmojis.getInstance().displayName(pack);
            graphics.text(font, shorten(displayName, font, PACK_COLUMN_WIDTH - 6),
        ...
        for (int i = from; i < to; i++) {
            ...
            graphics.text(font, display, tabX + 4, panelY + 3, selected ? 0xffffcc00 : 0xffffffff, true);
            tabX += font.width(display) + 10;
        }
        // page arrows
        if (pageCount > 1) {
            int btnX = gridX + gridWidth - 24;
            graphics.text(font, "<", btnX, panelY + 3, tabPage > 0 ? 0xffffffff : 0x66ffffff, true);
            graphics.text(font, ">", btnX + 12, panelY + 3, tabPage < pageCount - 1 ? 0xffffffff : 0x66ffffff, true);
        }
    ...
    /** Renders the meme through the font pipeline (code point + emoji font style), scaled to the cell */
    private void renderEmoji(Emoji emoji, GuiGraphicsExtractor graphics, int x, int y, int size) {
