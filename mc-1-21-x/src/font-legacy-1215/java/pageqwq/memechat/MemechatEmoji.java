...
public final class MemechatEmoji {
    ...
    public net.minecraft.client.gui.font.glyphs.BakedGlyph getGlyph() {
        updateLoadingState();
        return switch (state) {
            case LOADING, INACTIVE -> MemechatGlyphs.white();
            case ERROR -> MemechatGlyphs.error();
        ...
        static Future<MemechatGlyphProvider> load(ResourceLocation location) {
            return CompletableFuture.supplyAsync(() -> {
                    ...
                    .thenApplyAsync(image -> {
                        var texture = EmojiTexture.create("png_" + location.getPath().replace('/', '_'), image);
                        image.close();
                        float aspect = (float) texture.width / texture.height;
                        return (MemechatGlyphProvider) () -> new MemechatGlyph(
                                MemechatRenderTypesHolder.emojiTextured(texture.location), aspect);
            ...
            return CompletableFuture.supplyAsync(() -> {
                    ...
                    .thenApplyAsync(frames -> {
                        ...
                        for (int i = 0; i < frames.size(); i++) {
                            ...
                            var texture = EmojiTexture.create("gif_" + i, nativeImage);
                            nativeImage.close();
                            float aspect = (float) texture.width / texture.height;
                            glyphFrames.add(new MultiFrameGlyph.Frame(
                                    new MemechatGlyph(MemechatRenderTypesHolder.emojiTextured(texture.location), aspect),
                                    frame.delayMillis()));
                        }
    ...
    public interface MemechatGlyphProvider {
        net.minecraft.client.gui.font.glyphs.BakedGlyph getGlyph();
    }
